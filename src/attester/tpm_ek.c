...
tool_rc set_ek_template(ESYS_CONTEXT *ectx, TPM2B_PUBLIC *input_public, struct createek_context *ctx) {
...
tool_rc create_ek_handle(ESYS_CONTEXT *ectx, struct createek_context *ctx) {
    ...
    if (ctx->flags.t) {
        tool_rc rc = set_ek_template(ectx, &ctx->objdata.in.public, ctx);
        if (rc != tool_rc_success) {
            return rc;
        }
    }
